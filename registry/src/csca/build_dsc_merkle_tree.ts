...
import { getLeaf, getLeafDsc } from '../../../common/src/utils/pubkeyTree';
...
function processCertificate(pemContent: string, filePath: string) {
    try {
        ...
        const finalPoseidonHash = getLeafDsc(pemContent);
...
async function buildCscaMerkleTree() {
    ...
    if (DEVELOPMENT_MODE) {
        const mockDsc = [
            '../common/src/mock_certificates/sha256_rsa_4096/mock_dsc.pem',
            '../common/src/mock_certificates/sha256_rsapss_4096/mock_dsc.pem',
            '../common/src/mock_certificates/sha1_rsa_4096/mock_dsc.crt',
            '../common/src/mock_certificates/sha1_rsa_3_4096/mock_dsc.pem',
            '../common/src/mock_certificates/sha256_rsa_3_4096/mock_dsc.crt',
            '../common/src/mock_certificates/sha384_rsa_65537_4096/mock_dsc.pem',
            '../common/src/mock_certificates/sha512_rsa_65537_4096/mock_dsc.pem',
        ];

        for (const mockDscFile of mockDsc) {
            try {
                const pemContent = fs.readFileSync(mockDscFile, 'utf8');
                const leafValue = processCertificate(pemContent, mockDscFile);
                if (leafValue) {
                    tree.insert(leafValue);
                }
            } catch (error) {
                console.error('\x1b[31m', `Error processing mock file ${mockDscFile}:`, '\x1b[0m');
            }
        }
    }
