        ...
        describe(`n=${n},k=${k}`, function () {
            ...
            for (let i = 0; i < num_tests; i++) {
                ...
                if (isEcdsa) {
                    ...
                    const xBinary = Array.from({ length: keyLength }, () => Math.random() < 0.5 ? '1' : '0').join('');
                    const yBinary = Array.from({ length: keyLength }, () => Math.random() < 0.5 ? '1' : '0').join('');
                    ...
                    const modulusBinary = Array.from({ length: keyLength }, () => Math.random() < 0.5 ? '1' : '0').join('');
            ...
            async function runTest(circuit, pubkey_dsc, pubkey_dsc_padded, salt, pubKey_csca_hash) {
                ...
                if (register_hash !== dsc_hash) {
                    ...
                    logTestFailure(n, k, keyLength, isEcdsa, pubkey_dsc, pubkey_dsc_padded, register_hash, dsc_hash);
                }
...
function logTestFailure(n, k, keyLength, isEcdsa, pubkey_dsc, pubkey_dsc_padded, register_hash, dsc_hash) {
