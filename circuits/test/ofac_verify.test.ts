...
let circuit: any;
let passportData = mockPassportData_sha256WithRSAEncryption_65537; //Mock passport is added in ofac list to test circuits
let tree : any;
...
describe("start testing ofac_passportNo_verifier.circom", function () {
    ...
    before(async () => {
        circuit = await wasm_tester(path.join(__dirname, "../circuits/ofac_passportNo_verifier.circom"),
            {
                include: [
                    "node_modules",
